function addScreenPositionFunction(p5Instance?: p5) {
    ...
    if (p.resetMatrix instanceof Function) {
        ...
        p.resetMatrix = function (...args: any[]) {
            if (context == R_2D) p._renderer.matrixStack = [new p5.Matrix()];
    ...
    if (p.applyMatrix instanceof Function) {
        ...
        p.applyMatrix = function (...args: any[]) {
            if (context == R_2D) {
                let stack = p._renderer.matrixStack;
                let m = last(stack);
                let sm = new p5.Matrix();
    ...
    p.screenPosition = function (x: number | p5.Vector | number[], y: number, z: number) {
    ...
    function getObjectName(obj: any) {
